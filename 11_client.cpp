...
static int32_t on_response(const uint8_t *data, size_t size)
{
    ...
    case SER_STR:
        ...
        else
        {
            ...
            printf("(str) %.*s\n", len, out.c_str());
            return 1 + 4 + len; // Return total processed bytes
        }
    case SER_INT:
        if (size < 1 + 8)
        {
        ...
        else
        {
            ...
            return 1 + 8; // Return the total number of bytes read
        }
    case SER_DBL:
        if (size < 1 + sizeof(double))
        {
            msg("bad response: incomplete double");
            return -1;
        }
        else
        {
            ...
            std::cout << "(dbl) " << std::fixed << std::setprecision(6) << val << std::endl;
...
void print_usage()
{
    std::cout << "Redis-like Client Usage:" << std::endl;
    ...
    std::cout << "    KEYS                       - Get all keys" << std::endl;
    std::cout << "\n  Sorted Set (ZSET) Commands:" << std::endl;
    std::cout << "    ZADD <key> <score> <member> [score member ...]  - Add members to a sorted set" << std::endl;
    std::cout << "    ZSCORE <key> <member>      - Get the score of a member in a sorted set" << std::endl;
    std::cout << "    ZRANGE <key> <start> <stop> [WITHSCORES]        - Get range of members by index" << std::endl;
...
int main(int argc, char **argv)
{
    ...
    std::cout << "Connected to Redis-like server at 127.0.0.1:3000" << std::endl;
    std::cout << "Type HELP for usage information or QUIT to exit" << std::endl;
    ...
    if (argc == 1)
    {
        ...
        while (running)
        {
            ...
            std::vector<std::string> cmd = split_args(input);

            // Handle local commands
            if (cmd[0] == "QUIT" || cmd[0] == "quit" || cmd[0] == "EXIT" || cmd[0] == "exit")
            {
                running = false;
                continue;
            }
    ...
    {
        // Command line mode - pass arguments as command
