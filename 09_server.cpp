...
static void do_get(std::vector<std::string> &cmd, std::string &out)
{
    ...
    if (!node)
    {
        out_err(out, RES_NX, "Key not found");
        return;
    }
...
static void do_set(std::vector<std::string> &cmd, std::string &out)
{
    ...
    if (node)
    {
        ...
        out_str(out, "Updated existing key's value,");
    }
    else
    {
        Entry *ent = new Entry();
        ...
        out_str(out, "Created new key with given value.");
    }
}
...
static bool try_one_request(Conn *conn)
{
    ...
    {
        out.clear();
        out_err(out, -1, "response is too big");
    }
